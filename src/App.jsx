  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a12", color: "#e8e4dc", fontFamily: "'Inter', system-ui, sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        background: "radial-gradient(ellipse at 20% 20%, rgba(139,92,246,0.08) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(109,40,217,0.06) 0%, transparent 50%), radial-gradient(ellipse at 50% 50%, rgba(0,0,0,0) 0%, #0a0a12 70%)"
      }} />
      ...
      {showUpgradeModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "1rem", backdropFilter: "blur(8px)" }}>
          <div className="card fade-in" style={{ padding: "2.5rem", maxWidth: 420, width: "100%", textAlign: "center" }}>
            <span className="loading-moon" style={{ animation: "moonFloat 2s ease-in-out infinite" }}>🌙</span>
        ...
        {tab === "create" ? (
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: "1.5rem" }}>
              <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && input.trim() && generate(input.trim())} placeholder="rainy Tokyo night, haunted library..." style={{ flex: 1, background: "rgba(20,20,30,0.8)", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: 12, color: "#e8e4dc", fontSize: mobile ? 16 : 15, padding: mobile ? "14px 16px" : "13px 16px", outline: "none", fontFamily: "inherit", transition: "border-color 0.2s, box-shadow 0.2s" }} />
                  ...
                  <div style={{ display: "flex", gap: mobile ? 4 : 8 }}>
                    {result.palette?.map((c, i) => (
                      <div key={i} style={{ flex: 1 }} onClick={() => copyToClipboard(c.hex, i)}>
                        <div className="swatch" style={{ height: mobile ? 60 : 80, borderRadius: 10, background: c.hex, cursor: "pointer", position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {copied === i && <span style={{ position: "absolute", fontSize: 16 }}>✓</span>}
                        </div>
                        {!mobile && <p style={{ margin: "6px 0 0", fontSize: 10, color: "#6b6870", textAlign: "center", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.name}</p>}
