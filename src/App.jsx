...
function ExampleBoard({ board, mobile }) {
  return (
    <div style={{ background: "#141418", border: "0.5px solid #2a2a2e", borderRadius: 14, padding: mobile ? "1.25rem" : "1.5rem", marginBottom: 16 }}>
      <div style={{ marginBottom: "1.25rem" }}>
        <h3 style={{ margin: 0, fontSize: mobile ? 18 : 22, fontWeight: 500, color: "#e8e4dc" }}>{board.title}</h3>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {board.palette.map((c, i) => (
            <div key={i} style={{ flex: 1 }}>
              <div style={{ height: mobile ? 48 : 56, borderRadius: 6, background: c.hex }} />
      ...
      <div style={{ marginBottom: "1.25rem" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {board.keywords.map((k, i) => (
            <span key={i} style={{ background: "#1e1c22", border: "0.5px solid #2a2a2e", borderRadius: 20, padding: "4px 10px", fontSize: mobile ? 11 : 12, color: "#c4c0cc" }}>{k}</span>
          ))}
        </div>
      </div>
    ...
    <div style={{ minHeight: "100vh", background: "#0d0d0f", color: "#e8e4dc", fontFamily: "system-ui, sans-serif" }}>
      ...
      {showUpgradeModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "1rem" }}>
          <div style={{ background: "#141418", border: "0.5px solid #3a3740", borderRadius: 16, padding: "2rem", maxWidth: 420, width: "100%", textAlign: "center" }}>
            <div style={{ fontSize: 40, marginBottom: 16 }}>🌙</div>
            <h2 style={{ margin: "0 0 8px", fontSize: mobile ? 18 : 22, color: "#e8e4dc", fontWeight: 500 }}>You've reached your free limit</h2>
      ...
      {/* Header */}
      <div style={{ borderBottom: "0.5px solid #2a2a2e", padding: mobile ? "1rem" : "1.5rem 2rem" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: mobile ? 10 : 0 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: mobile ? 18 : 22, fontWeight: 500, color: "#e8e4dc" }}>Mood Board</h1>
        ...
        {mobile && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            {user && !isPro && <div style={{ fontSize: 12, color: remaining > 0 ? "#6b6870" : "#f87171" }}>{remaining > 0 ? `${remaining} free left` : "No free left"}</div>}
            {user && isPro && <div style={{ fontSize: 12, color: "#8b5cf6" }}>✦ Pro</div>}
            {!isPro && <button onClick={() => window.open(STRIPE_LINK, "_blank")} style={{ background: "linear-gradient(135deg, #8b5cf6, #6d28d9)", border: "none", borderRadius: 8, color: "#fff", fontWeight: 500, fontSize: 12, padding: "6px 12px", cursor: "pointer" }}>Go Pro $7/mo</button>}
            {["create", "saved"].map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ background: tab === t ? "#1e1c22" : "transparent", border: `0.5px solid ${tab === t ? "#3a3740" : "#2a2a2e"}`, color: tab === t ? "#e8e4dc" : "#6b6870", borderRadius: 8, padding: "5px 12px", fontSize: 12, cursor: "pointer" }}>{t}</button>
            ))}
            {user ? (
              <button onClick={signOut} style={{ background: "transparent", border: "0.5px solid #2a2a2e", borderRadius: 8, color: "#6b6870", fontSize: 12, padding: "5px 12px", cursor: "pointer" }}>Sign out</button>
            ) : (
              <button onClick={signInWithGoogle} style={{ background: "#1e1c22", border: "0.5px solid #3a3740", borderRadius: 8, color: "#c4c0cc", fontSize: 12, padding: "5px 12px", cursor: "pointer" }}>Sign in</button>
            )}
          </div>
        )}
      </div>
      ...
      {tab === "create" ? (
        <div style={{ maxWidth: 720, margin: "0 auto", padding: mobile ? "1rem" : "2rem" }}>
          {user && !isPro && remaining <= 0 && (
            <div style={{ background: "#1a1015", border: "0.5px solid #8b5cf6", borderRadius: 12, padding: "1rem", marginBottom: "1rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
              <div>
              ...
              <button onClick={() => setShowUpgradeModal(true)} style={{ background: "linear-gradient(135deg, #8b5cf6, #6d28d9)", border: "none", borderRadius: 8, color: "#fff", fontWeight: 500, fontSize: 13, padding: "10px 16px", cursor: "pointer" }}>Upgrade Now</button>
            </div>
          )}

          {!user && (
            <div style={{ background: "#141418", border: "0.5px solid #2a2a2e", borderRadius: 12, padding: "1rem", marginBottom: "1rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
              <div>
              ...
              <button onClick={signInWithGoogle} style={{ background: "#1e1c22", border: "0.5px solid #3a3740", borderRadius: 8, color: "#c4c0cc", fontSize: 13, padding: "10px 16px", cursor: "pointer" }}>Sign in with Google</button>
            </div>
          )}

          <div style={{ display: "flex", gap: 8, marginBottom: "1.5rem" }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && input.trim() && generate(input.trim())} placeholder="rainy Tokyo night, haunted library..." style={{ flex: 1, background: "#141418", border: "0.5px solid #2a2a2e", borderRadius: 10, color: "#e8e4dc", fontSize: mobile ? 16 : 15, padding: mobile ? "14px 16px" : "12px 16px", outline: "none" }} />
            <button onClick={surpriseMe} disabled={loading} title="Surprise me!" style={{ background: "#1e1c22", border: "0.5px solid #3a3740", borderRadius: 10, color: "#c4c0cc", fontSize: 18, padding: "0 14px", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1, minWidth: 48 }}>🎲</button>
            <button onClick={() => input.trim() && generate(input.trim())} disabled={loading} style={{ background: !user ? "#3a3740" : remaining > 0 ? "#8b5cf6" : "#3a3740", border: "none", borderRadius: 10, color: "#fff", fontWeight: 500, fontSize: mobile ? 13 : 14, padding: "0 16px", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1, minWidth: mobile ? 80 : 100 }}>
          ...
          {result && !result.error && (
            <div id="mood-board-result">
              <div style={{ marginBottom: "1.5rem" }}>
                <h2 style={{ margin: 0, fontSize: mobile ? 22 : 28, fontWeight: 500, color: "#e8e4dc" }}>{result.title}</h2>
                <p style={{ margin: "6px 0 0", fontSize: mobile ? 13 : 15, color: "#9d9aa6", fontStyle: "italic" }}>{result.tagline}</p>
              </div>
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ display: "flex", gap: mobile ? 4 : 8 }}>
                  {result.palette?.map((c, i) => (
                    <div key={i} style={{ flex: 1 }}>
                      <div onClick={() => copyToClipboard(c.hex)} title="Click to copy" style={{ height: mobile ? 56 : 72, borderRadius: 8, background: c.hex, cursor: "pointer" }} />
                      {!mobile && <p style={{ margin: "6px 0 0", fontSize: 11, color: "#6b6870", textAlign: "center" }}>{c.name}</p>}
                      <p onClick={() => copyToClipboard(c.hex)} title="Click to copy" style={{ margin: "4px 0 0", fontSize: 9, color: "#4a4850", textAlign: "center", fontFamily: "monospace", cursor: "pointer" }}>{c.hex}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                  {result.keywords?.map((k, i) => (
                    <span key={i} style={{ background: "#141418", border: "0.5px solid #2a2a2e", borderRadius: 20, padding: mobile ? "4px 10px" : "5px 12px", fontSize: mobile ? 12 : 13, color: "#c4c0cc" }}>{k}</span>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: "1.5rem", background: "#141418", border: "0.5px solid #2a2a2e", borderRadius: 12, padding: "1.25rem 1.5rem" }}>
                <p style={{ margin: "0 0 8px", fontSize: 11, letterSpacing: "0.12em", color: "#6b6870", textTransform: "uppercase" }}>Scene</p>
                <p style={{ margin: 0, fontSize: mobile ? 13 : 14, color: "#c4c0cc", lineHeight: 1.75, fontStyle: "italic" }}>{result.sceneDescription}</p>
              </div>

              <div style={{ display: "flex", gap: 8, flexWrap: mobile ? "wrap" : "nowrap" }}>
                <button onClick={() => generate(input.trim())} style={{ background: "transparent", border: "0.5px solid #2a2a2e", borderRadius: 8, color: "#9d9aa6", fontSize: 13, padding: "10px 18px", cursor: "pointer", flex: mobile ? "1 1 auto" : "none" }}>Regenerate</button>
                <button onClick={handleSave} style={{ background: "#1e1c22", border: "0.5px solid #3a3740", borderRadius: 8, color: "#c4c0cc", fontSize: 13, padding: "10px 18px", cursor: "pointer", flex: mobile ? "1 1 auto" : "none" }}>Save</button>
                <button onClick={shareBoard} style={{ background: "#1e1c22", border: "0.5px solid #3a3740", borderRadius: 8, color: "#c4c0cc", fontSize: 13, padding: "10px 18px", cursor: "pointer", flex: mobile ? "1 1 auto" : "none" }}>{isPro ? "Export PNG" : "Share 📸"}</button>
              </div>
            </div>
          )}
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", padding: mobile ? "1rem" : "2rem" }}>
          {saved.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {saved.map(s => (
                <div key={s.id} onClick={() => { setResult(s); setInput(s.prompt); setTab("create"); }} style={{ background: "#141418", border: "0.5px solid #2a2a2e", borderRadius: 14, padding: mobile ? "1rem" : "1.25rem 1.5rem", cursor: "pointer" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                    <div>
                      <p style={{ margin: 0, fontWeight: 500, fontSize: mobile ? 14 : 16, color: "#e8e4dc" }}>{s.title}</p>
