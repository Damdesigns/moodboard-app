...
const QUIZ_QUESTIONS = [
  { q: "What temperature feels right?", options: ["🔥 Warm & intense", "❄️ Cold & distant", "🌙 Cool & mysterious", "☀️ Bright & alive"] },
  { q: "What time of day?", options: ["🌅 Golden hour", "🌃 Deep night", "🌫️ Grey afternoon", "🌄 Before dawn"] },
  { q: "Where are you?", options: ["🏙️ Urban & concrete", "🌿 Nature & wild", "🏛️ Historic & grand", "🚀 Otherworldly"] }
];
...
const styles = `
  ...
  * { box-sizing: border-box; }
  ...
  .quiz-option { background: rgba(20,20,30,0.8); border: 0.5px solid rgba(255,255,255,0.08); border-radius: 12px; color: #9d9aa6; cursor: pointer; transition: all 0.2s ease; font-family: 'Inter', system-ui, sans-serif; text-align: center; padding: 12px; font-size: 13px; }
  .quiz-option:hover { border-color: rgba(139,92,246,0.4); color: #e8e4dc; background: rgba(139,92,246,0.08); }
  .quiz-option.selected { border-color: rgba(139,92,246,0.6); color: #e8e4dc; background: rgba(139,92,246,0.15); }
          ...
          system: `You are a mood board curator. Given a mood or aesthetic, return ONLY a JSON object with no markdown, no preamble. Schema:
{
  "title": "string",
  "tagline": "string",
  "palette": [{ "hex": "#xxxxxx", "name": "string", "story": "one poetic sentence about what this color feels like" }],
  ...
  function handleQuizAnswer(answer) {
    ...
    if (quizStep < QUIZ_QUESTIONS.length - 1) {
      ...
      const prompt = `Generate a mood board aesthetic based on these preferences: ${Object.values(newAnswers).join(", ")}. Create something unique and evocative.`;
      setInput(prompt);
      generate(prompt);
      setQuizAnswers({});
      setQuizStep(0);
 ...
 async function exportPDF() {
  ...
  doc.setTextColor(157, 154, 166);
  doc.text(result.tagline, 20, y);
  y += 16;

  // Palette
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(107, 104, 112);
  doc.text("PALETTE", 20, y);
  y += 6;
  const swatchW = (W - 40) / result.palette.length;
  result.palette.forEach((c, i) => {
    const r = parseInt(c.hex.slice(1, 3), 16);
    const g = parseInt(c.hex.slice(3, 5), 16);
    const b = parseInt(c.hex.slice(5, 7), 16);
    doc.setFillColor(r, g, b);
    doc.roundedRect(20 + i * swatchW, y, swatchW - 3, 18, 2, 2, "F");
    doc.setTextColor(157, 154, 166);
    doc.setFontSize(7);
    doc.text(c.hex, 20 + i * swatchW + (swatchW - 3) / 2, y + 22, { align: "center" });
  });
  y += 30;
  ...
  y += 6;
  doc.setTextColor(196, 192, 204);
  doc.setFontSize(11);
  doc.text(result.keywords.join("  ·  "), 20, y, { maxWidth: W - 40 });
  y += 12;
  ...
  result.fonts.forEach(f => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(232, 228, 220);
    doc.text(`${f.name} — ${f.role}`, 20, y);
    y += 6;
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(107, 104, 112);
    doc.text(f.description, 20, y, { maxWidth: W - 40 });
    y += 10;
  });
  ...
  doc.setFont("helvetica", "italic");
  doc.setTextColor(196, 192, 204);
  const sceneLines = doc.splitTextToSize(result.sceneDescription, W - 40);
  doc.text(sceneLines, 20, y);
  y += sceneLines.length * 6 + 10;

  // Footer
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(74, 72, 80);
  doc.text("Generated by moodboardai.space", W / 2, 285, { align: "center" });

  doc.save(`${result.title || "moodboard"}.pdf`);
...
COLOR PALETTE
${result.palette?.map(c => `${c.name}: ${c.hex}${c.story ? `\n  "${c.story}"` : ""}`).join("\n")}

TYPOGRAPHY
${result.fonts?.map(f => `${f.name} (${f.role})\n  ${f.description}`).join("\n\n")}
...
${"=".repeat(50)}
Generated by Mood Board AI · moodboardai.space`;

    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${result.title || "brand-kit"}.txt`;
    a.click();
  ...
  if (presentMode && result) return (
    <div style={{ position: "fixed", inset: 0, background: "#0a0a12", zIndex: 1000, display: "flex", flexDirection: "column", padding: mobile ? "2rem 1.5rem" : "4rem", overflow: "auto" }} onClick={() => setPresentMode(false)}>
      <style>{styles}</style>
      <div style={{ maxWidth: 900, margin: "0 auto", width: "100%" }}>
        <p style={{ fontSize: 11, color: "#4a4850", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 32 }}>Click anywhere to exit · moodboardai.space</p>
        <h1 style={{ fontSize: mobile ? 48 : 80, fontWeight: 700, letterSpacing: "-2px", color: "#e8e4dc", margin: "0 0 12px", lineHeight: 1 }}>{result.title}</h1>
        <p style={{ fontSize: mobile ? 16 : 22, color: "#6b6870", fontStyle: "italic", marginBottom: 48 }}>{result.tagline}</p>
        <div style={{ display: "flex", gap: 12, marginBottom: 48 }}>
          {result.palette?.map((c, i) => (
            <div key={i} style={{ flex: 1 }}>
              <div style={{ height: mobile ? 80 : 140, borderRadius: 12, background: c.hex }} />
              <p style={{ margin: "8px 0 0", fontSize: 10, color: "#4a4850", textAlign: "center", fontFamily: "monospace" }}>{c.hex}</p>
              {!mobile && c.story && <p style={{ margin: "4px 0 0", fontSize: 10, color: "#6b6870", textAlign: "center", fontStyle: "italic", lineHeight: 1.3 }}>{c.story}</p>}
            </div>
          ))}
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 48 }}>
          {result.keywords?.map((k, i) => (
            <span key={i} style={{ background: "rgba(139,92,246,0.08)", border: "0.5px solid rgba(139,92,246,0.2)", borderRadius: 20, padding: "8px 18px", fontSize: mobile ? 14 : 16, color: "#c4c0cc" }}>{k}</span>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 16, marginBottom: 48 }}>
          {result.fonts?.map((f, i) => (
            <div key={i} style={{ background: "rgba(139,92,246,0.05)", border: "0.5px solid rgba(139,92,246,0.1)", borderRadius: 14, padding: "20px 24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                <span style={{ fontSize: mobile ? 18 : 22, fontWeight: 600, color: "#e8e4dc" }}>{f.name}</span>
                <span style={{ fontSize: 10, background: "rgba(139,92,246,0.15)", borderRadius: 4, padding: "3px 8px", color: "#a78bfa", textTransform: "uppercase" }}>{f.role}</span>
              </div>
              <p style={{ margin: 0, fontSize: 14, color: "#6b6870", lineHeight: 1.5 }}>{f.description}</p>
            </div>
          ))}
        </div>
        <div style={{ background: "rgba(139,92,246,0.05)", border: "0.5px solid rgba(139,92,246,0.1)", borderRadius: 14, padding: "24px 28px" }}>
          <p style={{ margin: "0 0 10px", fontSize: 10, letterSpacing: "0.2em", color: "#6b6870", textTransform: "uppercase", fontWeight: 500 }}>Scene</p>
          <p style={{ margin: 0, fontSize: mobile ? 15 : 18, color: "#c4c0cc", lineHeight: 1.9, fontStyle: "italic" }}>{result.sceneDescription}</p>
        </div>
      </div>
    </div>
  );
      ...
      {showUpgradeModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "1rem", backdropFilter: "blur(8px)" }}>
          <div className="card fade-in" style={{ padding: "2.5rem", maxWidth: 420, width: "100%", textAlign: "center" }}>
            ...
            <p style={{ margin: "0 0 24px", fontSize: 14, color: "#9d9aa6", lineHeight: 1.6 }}>Upgrade to Pro for unlimited mood boards, unlimited saves, clean PNG exports, brand kit downloads, presentation mode and full access — just $7/month.</p>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 720, margin: "0 auto", padding: mobile ? "1rem" : "2rem" }}>
        {tab === "create" ? (
          <>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: showQuiz ? "1rem" : "1.5rem" }}>
            ...
            {!showQuiz && !result && !loading && (
              <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
                <button onClick={() => setShowQuiz(true)} style={{ background: "transparent", border: "0.5px solid rgba(139,92,246,0.3)", borderRadius: 20, color: "#9d9aa6", fontSize: 13, padding: "8px 20px", cursor: "pointer", fontFamily: "inherit" }}>
                  🤔 Not sure what you want? Let me help →
                </button>
              </div>
            )}
            ...
            {showQuiz && (
              <div className="card fade-in" style={{ padding: "1.5rem", marginBottom: "1.5rem" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
                  <div>
                    <p style={{ margin: 0, fontSize: 11, color: "#6b6870", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 500 }}>Question {quizStep + 1} of {QUIZ_QUESTIONS.length}</p>
                    <p style={{ margin: "4px 0 0", fontSize: mobile ? 15 : 17, fontWeight: 600, color: "#e8e4dc" }}>{QUIZ_QUESTIONS[quizStep].q}</p>
                  </div>
                  <button onClick={() => { setShowQuiz(false); setQuizStep(0); setQuizAnswers({}); }} style={{ background: "transparent", border: "none", color: "#4a4850", fontSize: 18, cursor: "pointer" }}>✕</button>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {QUIZ_QUESTIONS[quizStep].options.map((opt, i) => (
                    <button key={i} className={`quiz-option${quizAnswers[quizStep] === opt ? " selected" : ""}`} onClick={() => handleQuizAnswer(opt)}>{opt}</button>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 4, marginTop: 12, justifyContent: "center" }}>
                  {QUIZ_QUESTIONS.map((_, i) => (
                    <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: i <= quizStep ? "#8b5cf6" : "rgba(255,255,255,0.1)" }} />
                  ))}
                </div>
              </div>
            )}
              ...
              <div id="mood-board-result" className="fade-in">
                  ...
                  <div style={{ display: "flex", gap: mobile ? 4 : 8 }}>
                    {result.palette?.map((c, i) => (
                      <div key={i} style={{ flex: 1 }} onClick={() => copyToClipboard(c.hex, i)}>
                        <div className="swatch" style={{ height: mobile ? 60 : 80, borderRadius: 10, background: c.hex, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}
                          onMouseEnter={e => { const el = e.currentTarget.querySelector('.color-story'); if (el) el.style.opacity = 1; }}
                          onMouseLeave={e => { const el = e.currentTarget.querySelector('.color-story'); if (el) el.style.opacity = 0; }}
                        >
                          {copied === i && <span style={{ fontSize: 16, color: "#fff", textShadow: "0 1px 4px rgba(0,0,0,0.5)", zIndex: 2 }}>✓</span>}
                          {c.story && (
                            <div className="color-story" style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", padding: 8, opacity: 0, transition: "opacity 0.2s ease", borderRadius: 10, zIndex: 1 }}>
                              <p style={{ margin: 0, fontSize: 9, color: "#fff", textAlign: "center", lineHeight: 1.4, fontStyle: "italic" }}>{c.story}</p>
                            </div>
                          )}
                        </div>
                ...
                <div className="card" style={{ padding: "1.25rem", marginBottom: "1.5rem", background: "rgba(139,92,246,0.05)", borderColor: "rgba(139,92,246,0.15)" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    <p style={{ margin: 0, fontSize: 10, letterSpacing: "0.15em", color: "#6b6870", textTransform: "uppercase", fontWeight: 500 }}>Scene</p>
                    <button onClick={() => { navigator.clipboard.writeText(result.sceneDescription); showToast("✓ Scene copied!"); }} style={{ background: "transparent", border: "0.5px solid rgba(139,92,246,0.2)", borderRadius: 6, color: "#9d9aa6", fontSize: 11, padding: "3px 10px", cursor: "pointer", fontFamily: "inherit" }}>Copy</button>
                  </div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: mobile ? "wrap" : "nowrap" }}>
                  ...
                  <button className="btn-secondary" onClick={exportBrandKit} style={{ fontSize: 13, padding: "10px 18px", flex: mobile ? "1 1 auto" : "none", color: isPro ? "#a78bfa" : "#6b6870" }}>{isPro ? "Brand Kit ✦" : "Brand Kit 🔒"}</button>
                  <button className="btn-secondary" onClick={() => isPro ? setPresentMode(true) : setShowUpgradeModal(true)} style={{ fontSize: 13, padding: "10px 18px", flex: mobile ? "1 1 auto" : "none", color: isPro ? "#a78bfa" : "#6b6870" }}>{isPro ? "Present ✦" : "Present 🔒"}</button>
                </div>
              </div>
            )}

            {!result && !loading && !showQuiz && (
